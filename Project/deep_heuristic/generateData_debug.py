...
def get_rand(low=0.1, high=0.10):
...
class PlanningScenario(object):
    ...
    def reset(self):
        ...
        with HideOutput():
            ...
            obj1 = create_box(get_rand(), get_rand(), 0.2, mass=0.5, color=(0.859, 0.192, 0.306, 1.0))
            obj2 = create_box(get_rand(), get_rand(), 0.2, mass=0.5, color=(0.271, 0.706, 0.490, 1.0))
            obj3 = create_box(get_rand(), get_rand(), 0.2, mass=0.5, color=(0.647, 0.498, 0.894, 1.0))
...
def gather_training_data():
    ...
    file_reach = 'false_negative.pk'
    if os.path.exists(file_reach):
        with open(file_reach, 'rb') as f:
            tdata_falsenegativ = pk.load(f)

    print('existing_record, false negative = {}'.format(len(tdata_falsenegativ)))
    ...
    for ep in range(500):
        ...
        for index, body in enumerate(scn.movable_bodies):
            body_info = scn.dic_body_info[body]

            ellipsoid_frame, obj_extent, list_dist, list_dir_jj, list_z_jj = get_ellipsoid_frame(body, body_info, robot)
            mat_image = None
            for direction in range(1, 2):
                ...
                _, command, _, q_grasp = f_ik_grasp.search((body, body_pose, grasp))
                # if the object is reachable
                if q_grasp:
                    ...
                    if np.min(mat_image[16:48, -8:]) > 0.5 and not label:
                        ...
                        #time.sleep(10)
                        tdata_falsenegativ.append((WorldSaver(), mat_image, index))
        with open(file_reach, 'wb') as f:
            pk.dump(tdata_falsenegativ, f)
...
def world_loader():
    ...
    print(len(tdata_falsenegativ))
    for world in tdata_falsenegativ:
        world = tdata_falsenegativ[1]
        ...
        plt.imshow(world[1], 'gray', vmin=0, vmax=1)
        plt.show()

        body = scn.movable_bodies[world[2]]
        body_info = scn.dic_body_info[body]
        ellipsoid_frame, obj_extent, list_dist, list_dir_jj, list_z_jj = get_ellipsoid_frame(body, body_info, robot)

        grasp_dir = GraspDirection(body, 1)
        grasp = f_sample_grasp.search((body, grasp_dir))[0]
        ...
        if visualization:
            draw_shouldercenter_frame(robot, 300)
            draw_frame(ellipsoid_frame, 300)
