import time
# dump_body(robot)
from .pr2_utils import get_top_grasps, get_side_grasps, get_sucker_grasps, get_sucker_grasp_top, get_top_cylinder_grasps
...
class sdg_sample_grasp_top(object):
    def __init__(self, robot):
        self.robot = robot
        self.end_effector_link = link_from_name(robot, TOOL_FRAMES[get_body_name(robot)])
...
class sdg_sample_grasp_cylinder(object):
    def __init__(self, robot):
        self.robot = robot
        self.end_effector_link = link_from_name(robot, TOOL_FRAMES[get_body_name(robot)])
    ...
    def __call__(self, input_tuple, seed=None):
        ...
        grasp_poses = get_top_cylinder_grasps(body)
        grasp_pose = random.sample(grasp_poses, 1)[0]
        ...
        for _ in range(self.num_attempts):
            ...
            if self.teleport:
                ...
                path = plan_direct_joint_motion(self.robot, approach_conf.joints, q_grasp,
                                                obstacles=obstacles, disabled_collisions=DISABLED_COLLISION_PAIR)
                if path is None:
                    if DEBUG_FAILURE:
